...
import (
	...
	"github.com/etyvrox/offensiveboar/v3/pkg/sources/circleci"
)
...
func (e *Engine) ScanCircleCI(ctx context.Context, token string) (sources.JobProgressRef, error) {
	...
	sourceName := "offensiveboar - Circle CI"
	sourceID, jobID, _ := e.sourceManager.GetIDs(ctx, sourceName, circleci.SourceType)

	circleSource := &circleci.Source{}
	if err := circleSource.Init(ctx, "offensiveboar - Circle CI", jobID, sourceID, true, &conn, runtime.NumCPU()); err != nil {
		return sources.JobProgressRef{}, err
	}
